...
func TestDecide_MatchedGroupsIsIntersectionNotFullGroupList(t *testing.T) {
	...
	d := p.Decide(Principal{User: "alice", Groups: []string{"dba", "engineering"}}, Target{Host: "db1.lab.local", Port: 22}, nil)
	if !d.Allow {
...
func TestDecideHost_MatchedGroupsSet(t *testing.T) {
	p := demoGroupsPolicy()
	d := p.DecideHost(Principal{User: "alice", Groups: []string{"dba-oncall"}}, "db1.lab.local", nil)
...
func TestDecide_MatchedGroupsEmptyOnDeny(t *testing.T) {
	p := demoGroupsPolicy()
	d := p.Decide(Principal{User: "mallory", Groups: []string{"engineering"}}, Target{Host: "db1.lab.local", Port: 22}, nil)
