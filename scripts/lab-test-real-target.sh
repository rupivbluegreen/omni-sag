...
# A second role granting the SAME container under a different host spelling
# ("localhost" rather than "127.0.0.1"), pinning NO target_user and opting into
# a client-nameable account via allow_target_users. A different host string
# keeps every rule above unambiguous, so tests 1-6 are unaffected, while
# "alice%svc_db1@localhost" reaches the GRANT path: the account comes from the
# client and is authorized by the allow-list, not named by the rule.
for role in cfg["policy"]["roles"]:
    if role["name"] == "dba":
        demo = [r for r in role["allow"] if r.get("target_user") == "svc_db1"][0]
        break
cfg["policy"]["roles"].append({
    "name": "dbaself",
    "groups": role["groups"],
    "allow": [{
        "host": "localhost",
...
def main():
    ...
    # start bounds the search so a pattern already echoed earlier in the
    # transcript cannot satisfy a later wait. The "user%targetuser@host" login
    # form puts the target account in the GATEWAY prompt line too, so the
    # target-prompt wait below must only look at bytes that arrived after the
    # gateway password was sent.
    def wait_for(pattern, start=0):
        nonlocal buf
        pat = pattern.encode()
        while True:
            if buf.find(pat, start) >= 0:
                return True
...
# --- Test 7: an allow-listed account on a rule that pins nothing ------------
# The dbaself role grants "localhost" with no target_user and
# allow_target_users: ["svc_db1"], so this exercises the GRANT path: the
# account comes from the client and is authorized by the allow-list rather
# than named by the rule.
echo "== allow-listed client-supplied account on a rule that pins none =="
MARKER7="OMNISAG7_$$_$(date +%s)"
ALLOWED_OUT="$(python3 "$WORKDIR/ssh_shell.py" "$GW_PORT" "${GW_USER}%${TARGET_USER}@localhost@${TARGET_HOST}" \
  "$GW_PASSWORD" "$TARGET_PASSWORD" "BEGIN_${MARKER7}" "END_${MARKER7}" 2>"$WORKDIR/ssh_allowed.log")"
ALLOWED_WHOAMI="$(printf '%s\n' "$ALLOWED_OUT" | sed -n 's/^WHOAMI_RESULT=//p')"
if [ "$ALLOWED_WHOAMI" != "$TARGET_USER" ]; then
  fail "allow-listed client-supplied account was not honoured, whoami got: '$ALLOWED_WHOAMI'"
  echo "--- driver transcript ---" >&2; tail -n 40 "$WORKDIR/ssh_allowed.log" >&2
  echo "--- gateway log ---" >&2; tail -n 40 "$WORKDIR/gateway.log" >&2
  exit 1
fi
pass "alice%${TARGET_USER}@localhost accepted via allow_target_users on a rule pinning no account"

# --- Test 8: an account the allow-list does not name is still refused -------
echo "== an account outside allow_target_users is refused =="
NOTLISTED_OUT="$(python3 "$WORKDIR/ssh_shell.py" "$GW_PORT" "${GW_USER}%root@localhost@${TARGET_HOST}" \
  "$GW_PASSWORD" "$TARGET_PASSWORD" "BEGIN_NL_$$" "END_NL_$$" 2>"$WORKDIR/ssh_notlisted.log" || true)"
if printf '%s\n' "$NOTLISTED_OUT" | grep -q "^WHOAMI_RESULT="; then
  fail "an account outside allow_target_users was honoured: $NOTLISTED_OUT"
  echo "--- driver transcript ---" >&2; tail -n 40 "$WORKDIR/ssh_notlisted.log" >&2
  exit 1
fi
pass "alice%root@localhost refused: root is not in allow_target_users"
